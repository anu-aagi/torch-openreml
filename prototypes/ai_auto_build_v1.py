...
def build_fn(theta):
    ...
    # dSigma[i] has shape (*Sigma.shape) — gradient of each Sigma element w.r.t. theta[i]
    # Full Jacobian shape: (*Sigma.shape, *theta.shape)
    J = torch.autograd.functional.jacobian(compute_Sigma, theta)  # shape (*Sigma.shape, 3)

    # Split along last dim to match original dSigma list structure
