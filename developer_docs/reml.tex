...
covariance matrix \(\symbf{V}(\symbf{\theta})\), the BLUE of
...
\(\partial \symbf{P}/\partial \theta_j = -\symbf{P}\,\dot{\symbf{V}}_j\,\symbf{P}\),
...
\(\mathbb{E}[\symbf{Y}\symbf{Y}^\top] = \symbf{V}\) for the remainder of
this derivation.
...
zero-mean random vector with covariance \(\symbf{V}\), the identity
\(\mathbb{E}[\symbf{Y}^\top \symbf{A} \symbf{Y}] = \operatorname{tr}(\symbf{A} \symbf{V})\)
holds for any conformable matrix \(\symbf{A}\). Applying this with
\(\symbf{A} = \symbf{P}\,\dot{\symbf{V}}_k\,\symbf{P}\,\dot{\symbf{V}}_j\,\symbf{P}\)
...
\(\symbf{P}\symbf{V}\symbf{P} = \symbf{P}\), which can be verified by
direct substitution of the definition
\(\symbf{P} = \symbf{V}^{-1} - \symbf{V}^{-1}\symbf{X}(\symbf{X}^\top\symbf{V}^{-1}\symbf{X})^{-1}\symbf{X}^\top\symbf{V}^{-1}\).
...
\(\symbf{P}\symbf{V}\symbf{P} = \symbf{P}\), \begin{align}
...
\(\ddot{\symbf{V}}_{kj}\) cancel identically. Moreover, the trace is
symmetric in its matrix arguments, so \begin{equation}
...
\(\symbf{V}(\symbf{\theta}) = \sum_{t=1}^p \theta_t \symbf{M}_t\) with
known matrices \(\symbf{M}_t\), the second derivatives vanish:
\(\ddot{\symbf{V}}_{kj} = \symbf{0}\) for all \(k,j\). The average
...
\item
  Another term that commonly appears is \(\symbf{P} \dot{\symbf{V}}_k\).
  Expanding it gives: \begin{equation}
...
As defined in Equation \eqref{eq-sigma}, the covariance structure of the
Gaussian linear mixed model depends on \(\symbf{G}(\symbf{\theta})\) and
\(\symbf{R}(\symbf{\theta})\). The marginal covariance matrix
\(\symbf{V}\) can be decomposed into a sum of component-specific
contributions. Specifically, the model can be written as
\begin{equation}
\symbf{V} = \sum_{m=1}^{M} \symbf{Z}\symbf{G}_m(\symbf{\theta})\symbf{Z}^\top + \symbf{R}(\symbf{\theta}).
\end{equation}

For each random-effect covariance component
\(\symbf{G}_m(\symbf{\theta})\) and the residual covariance
\(\symbf{R}(\symbf{\theta})\), a number of standard covariance
structures are commonly employed. In the following, we summarise several
of these structures together with their parameterisations and
corresponding derivatives.

\subsection{Identity Matrix}\label{identity-matrix}

The identity covariance structure is defined as \begin{equation}
\symbf{G}(\symbf{\theta}) = \symbf{I}_n,
\end{equation} where \(\symbf{I}_n\) denotes the \(n \times n\) identity
matrix. This structure contains no free parameters, and therefore
represents a fixed covariance specification. Since there are no
parameters associated with this form, no derivative with respect to
\(\symbf{\theta}\) is required or defined.

\subsection{Scalar Matrix}\label{scalar-matrix}

The scalar covariance structure is defined as \begin{equation}
\symbf{G}(\symbf{\theta}) = \sigma^2 \symbf{I}_n,
\end{equation} where \(\sigma^2 > 0\) is a variance parameter. To ensure
positivity, we adopt the reparameterisation \begin{equation}
\theta = \log(\sigma),
\end{equation} which implies \begin{equation}
\sigma^2 = \exp(2\theta).
\end{equation}

Under this parameterisation, the derivative of the covariance matrix
with respect to \(\theta\) is given by \begin{equation}
...
\subsection{Diagnoal Matrix}\label{diagnoal-matrix}

The diagonal covariance structure extends the scalar case by allowing
heterogeneous variances across components while maintaining independence
between them. It is defined as \begin{equation}
\symbf{G}(\symbf{\theta}) = \mathrm{diag}(\sigma_1^2, \sigma_2^2, \ldots, \sigma_n^2),
\end{equation} where each \(\sigma_i^2 > 0\) represents a
component-specific variance parameter.

To ensure positivity of all variance terms, we adopt the element-wise
reparameterisation \begin{equation}
...
The derivative with respect to the \(k\)-th parameter \(\theta_k\) is a
diagonal matrix with a single non-zero entry corresponding to the
\(k\)-th diagonal element, given by \begin{equation}
\frac{\partial \symbf{G}}{\partial \theta_k}
= 2\exp(2\theta_k) \mathbf{e}_k \mathbf{e}_k^\top
= 2\sigma_k^2 \mathbf{e}_k \mathbf{e}_k^\top,
\end{equation} where \(\mathbf{e}_k\) denotes the \(k\)-th standard
basis vector in \(\mathbb{R}^n\).

\subsection{Compound Symmetric Matrix}\label{compound-symmetric-matrix}

The compound symmetric (CS) covariance structure assumes homogeneous
marginal variance and a common correlation across all pairs of
observations. It is defined as \begin{equation}
\symbf{G}(\symbf{\theta}) = \sigma^2 \left[(1 - \rho)\symbf{I}_n + \rho \mathbf{1}_n \mathbf{1}_n^\top \right],
\end{equation} where \(\sigma^2 > 0\) denotes the variance parameter,
\(\rho\) is the common correlation, \(\symbf{I}_n\) is the
\(n \times n\) identity matrix, and \(\mathbf{1}_n\) is an
\(n\)-dimensional vector of ones.

To ensure both positivity of \(\sigma^2\) and admissibility of the
correlation structure, we adopt the reparameterisations \begin{equation}
\theta_1 = \log(\sigma), \qquad \theta_2 \in \mathbb{R},
\end{equation} together with \begin{equation}
\sigma^2 = \exp(2\theta_1),
\end{equation} and a constrained mapping for the correlation parameter
given by \begin{equation}
\rho = \rho_{\min} + (1 - \rho_{\min})~ \mathrm{sigmoid}(\theta_2),
\qquad \rho_{\min} = -\frac{1}{n - 1}.
\end{equation}

This transformation guarantees that \(\rho \in (\rho_{\min}, 1)\),
ensuring that \(\symbf{G}(\symbf{\theta})\) remains positive
semidefinite for all admissible parameter values.
...
The derivative with respect to \(\theta_1\) is given by \begin{equation}
...
\end{equation} and therefore the derivative of the covariance matrix is
\begin{equation}
...
\subsection{AR(1) Matrix}\label{ar1-matrix}

The autoregressive order-1 (AR(1)) covariance structure assumes that
correlations decay exponentially with distance between indices. It is
commonly used to model ordered random effects or longitudinal data with
equally spaced observations. The covariance matrix is defined as
\begin{equation}
\symbf{G}(\symbf{\theta}) = \sigma^2 \symbf{T}(\rho),
\end{equation} where \(\sigma^2 > 0\) is a variance parameter and
\(\symbf{T}(\rho)\) is the correlation matrix with entries
\begin{equation}
\left[\symbf{T}(\rho)\right]_{ij} = \rho^{|i-j|}, \qquad i,j = 1,\ldots,n,
\end{equation} with \(|\rho| < 1\) ensuring positive definiteness.

To guarantee valid parameter constraints, we adopt the
reparameterisation \begin{equation}
\theta_1 = \log(\sigma), \qquad \theta_2 \in \mathbb{R},
\end{equation} together with \begin{equation}
\sigma^2 = \exp(2\theta_1),
\end{equation} and a bounded transformation for the correlation
parameter \begin{equation}
\rho = 2~\mathrm{sigmoid}(\theta_2) - 1,
\end{equation} which ensures \(\rho \in (-1,1)\).
...
For the correlation parameter, we first note that \begin{equation}
\frac{\partial \rho}{\partial \theta_2}
= 2~\mathrm{sigmoid}(\theta_2)\big(1 - \mathrm{sigmoid}(\theta_2)\big),
\end{equation} and the derivative of the correlation matrix has entries
\begin{equation}
\left[\frac{\partial \symbf{T}}{\partial \rho}\right]_{ij}
= |i-j| \rho^{|i-j|-1}.
\end{equation}

Therefore, the derivative of the covariance matrix with respect to
\(\theta_2\) is \begin{equation}
\frac{\partial \symbf{G}}{\partial \theta_2}
= \sigma^2 \frac{\partial \rho}{\partial \theta_2}
\frac{\partial \symbf{T}(\rho)}{\partial \rho}.
\end{equation}

This structure captures decaying dependence with increasing lag distance
and provides a parsimonious representation of serial correlation through
