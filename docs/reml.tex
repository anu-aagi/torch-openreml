...
model, \(\mathbb{E}[\mathbf{Y}] = \mathbf{X}\symbf{\beta}\). However, a
fundamental property of the projection matrix \(\mathbf{P}\) is that
\(\mathbf{P}\mathbf{X} = \mathbf{0}\), which implies
\(\mathbf{P}\mathbf{Y} = \mathbf{P}(\mathbf{Y} - \mathbf{X}\symbf{\beta})\).
Consequently, when computing expectations of expressions involving
\(\mathbf{P}\), we may without loss of generality treat \(\mathbf{Y}\)
as centered at zero. We therefore take
\(\mathbb{E}[\mathbf{Y}] = \mathbf{0}\) and
\(\mathbb{E}[\mathbf{Y}\mathbf{Y}^\top] = \symbf{\Sigma}\) for the
...
The quantity \(T_1\) is a quadratic form in \(\mathbf{Y}\). For a
zero-mean random vector with covariance \(\symbf{\Sigma}\), the identity
\(\mathbb{E}[\mathbf{Y}^\top \mathbf{A} \mathbf{Y}] = \operatorname{tr}(\mathbf{A} \symbf{\Sigma})\)
holds for any conformable matrix \(\mathbf{A}\). Applying this with
\(\mathbf{A} = \mathbf{P}\,\dot{\symbf{\Sigma}}_k\,\mathbf{P}\,\dot{\symbf{\Sigma}}_j\,\mathbf{P}\)
...
\end{equation} A key identity satisfied by \(\mathbf{P}\) is
\(\mathbf{P}\symbf{\Sigma}\mathbf{P} = \mathbf{P}\), which can be
verified by direct substitution of the definition
\(\mathbf{P} = \symbf{\Sigma}^{-1} - \symbf{\Sigma}^{-1}\mathbf{X}(\mathbf{X}^\top\symbf{\Sigma}^{-1}\mathbf{X})^{-1}\mathbf{X}^\top\symbf{\Sigma}^{-1}\).
...
\(\mathbf{P}\symbf{\Sigma}\mathbf{P} = \mathbf{P}\), \begin{align}
...
\(\symbf{\Sigma}(\symbf{\theta}) = \sum_{t=1}^p \theta_t \mathbf{V}_t\)
with known matrices \(\mathbf{V}_t\), the second derivatives vanish:
\(\ddot{\symbf{\Sigma}}_{kj} = \mathbf{0}\) for all \(k,j\). The average
...
During computation, we will avoid forming \(\mathbf{P}\) as it is a
complex matrix, so we will compute combined terms.

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  In both the score and AI matrix, the term \(\mathbf{P}\mathbf{Y}\)
  ...
  \end{equation} Since \(\mathbf{P}\) is a residual operator, this is
...
\item
  ...
  \(\mathbf{P} \dot{\symbf{\Sigma}}_k\). Expanding it gives:
  \begin{equation}
...
\(\mathbf{L}_{\mathcal{A}}\) (the Cholesky factor of \(\mathcal{A}\))
and then solve \(\mathcal{A}^{-1} \mathbf{S}\) efficiently via
back-substitution. Additionally, a damping factor can be applied to
\(\mathbf{D} \mathcal{A}^{-1} \mathbf{S}\) to control the learning rate.
